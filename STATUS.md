...
| **Action / Skill runtime** | ✅ Works | Skill registry, 7-risk approval gate (one-shot, payload-hash-bound, fail-closed TTL), redacting proof bundles, audit log, MCP-ready boundary. `familyclaw-actions` (270+ tests). **Seven genuinely functional skills**, all registered in `register_default_skills`: `fs_read` (allowlisted local file read), `web_fetch` (read-only HTTP GET, SSRF-guarded), `web_search` (keyless search engine query, SSRF-guarded, read-only), `file_write` (real disk write behind a canonicalized allowlist + `AlwaysRequireApproval`; proof records a path hash + byte count, never content), `research` (multi-source fetch + host-dedup + injection-escaped markdown report), `file_patch` (real unified-diff apply to disk behind the same canonicalized allowlist; `WriteLocal` + `AlwaysRequireApproval`; proof records a path hash + line-change count, never content), and `github_issue_draft` (deterministic issue draft plus optional real write of a Markdown draft artifact to an allowlisted path — no network, no credentials; `WriteExternal` + `RequireApproval`; proof records a path hash + byte count, never body). The remaining two provider examples (`email_triage`, `discord_thread_summary`) stay deterministic contract stubs because a live body would need an external API + credentials (Gmail, Discord). |
...
| **Slack channel MVP** | ⚠️ Outbound only | Web API `chat.postMessage` works (`FAMILYCLAW_CHANNEL_KIND=slack` + `SLACK_BOT_TOKEN` + `FAMILYCLAW_SLACK_CHANNEL_ID`), plus the `format_approval_prompt` helper. Feature `slack`; `docs/slack-setup.md`. **Inbound does not work yet:** no Events API / Socket Mode, and `POST /inject` is wired to the Discord channel only — in Slack mode it answers `503 discord channel not configured`. `doctor` also does not know the `slack` kind yet (it still checks the Telegram env vars and exits 1). |
| **Reliability Console** | ✅ Works | Gateway `/console` — Hermes-style Now strip + SSE audit feed + one-click approvals (`docs/CONSOLE.md`). |
| **Native OIDC JWT auth** | ✅ Works | Optional fail-closed OIDC (HS256 or JWKS) alongside `FAMILYCLAW_GATEWAY_TOKEN`; see `docs/ENTERPRISE_AUTH.md`. |
| **Postgres journal** | ⚠️ Library only | `PostgresJournal` implements the same `Journal` contract as `FileJournal` on a single `familyclaw_journal` table (feature `familyclaw-durable/postgres`, `DATABASE_URL`, single-tenant). **Not selectable at runtime:** no gateway/runtime path constructs it, so `serve` always opens a `FileJournal` even when `DATABASE_URL` is set. Usable today only by embedding the crate. |
...
| **Time Machine (inspect / fork / diff)** | ✅ Works | Deterministic journal rewind, fork with audit marker, counterfactual dry-run (`DryRunRecorder` has no dispatch path by construction), step-by-step timeline diff. `familyclaw_durable::TimeMachine` + `familyclaw replay inspect\|fork\|diff` CLI. Replay-proven promotion evidence (`familyclaw-growth::evidence`, fail-closed, no apply path). |
...
| **Semantic retrieval turned live** | 🚧 Infra ready, weight off by default | Embedding infra (`familyclaw-embeddings`, local candle MiniLM path), `VectorStore` trait, and cosine retrieval exist and are tested. `semantic_weight` is only turned on where a labeled recall fixture empirically shows semantic Hit@k > keyword Hit@k; otherwise ships keyword + provenance + temporal with semantic OFF (honest, not a loss). |
| **Real provider skill integrations** | 🚧 Two remaining stubs (`email_triage`, `discord_thread_summary`) | `file_patch` and `github_issue_draft` are now **real, working skills** with genuine gated on-disk side effects (unified-diff apply and a Markdown draft artifact) behind the same canonicalized allowlist + approval gate + content-free proof machinery as `file_write` — no network, no credentials. The remaining two examples (`email_triage`, `discord_thread_summary`) are still complete, tested implementations of the skill *contract* (manifest, risk class, approval policy, schema, taint) using deterministic placeholder data; a live body needs an external API + credentials (Gmail, Discord), so wiring a real provider is a swap of the execution body while the approval gate + proof redaction + audit apply for free. |
...
- [docs/USERS.md](docs/USERS.md) · [docs/COMMERCIAL_OFFER.md](docs/COMMERCIAL_OFFER.md) · [docs/LAUNCH.md](docs/LAUNCH.md) — go-to-market.
