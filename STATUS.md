...
| **Slack channel MVP** | ✅ Works | Web API `chat.postMessage` + `/inject` inbound; approval prompt helper. Feature `slack`; `docs/slack-setup.md`. Not a full Events API socket mode yet. |
...
| **Postgres journal** | ✅ Works | Feature `familyclaw-durable/postgres` + `DATABASE_URL` — same `Journal` contract as `FileJournal` (single-tenant). |
