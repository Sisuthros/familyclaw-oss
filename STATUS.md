...
| **Action / Skill runtime** | ✅ Works | Skill registry, 7-risk approval gate (one-shot, payload-hash-bound, fail-closed TTL), redacting proof bundles, audit log, MCP-ready boundary. `familyclaw-actions` (270+ tests). **Five genuinely functional skills**, all registered in `register_default_skills`: `fs_read` (allowlisted local file read), `web_fetch` (read-only HTTP GET, SSRF-guarded), `web_search` (keyless search engine query, SSRF-guarded, read-only), `file_write` (real disk write behind a canonicalized allowlist + `AlwaysRequireApproval`; proof records a path hash + byte count, never content), and `research` (multi-source fetch + host-dedup + injection-escaped markdown report). The former four provider examples (`email_triage`, `github_issue_draft`, `file_patch`, `discord_thread_summary`) remain deterministic contract stubs. |
